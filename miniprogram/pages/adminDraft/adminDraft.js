...
Page({
  ...
  async deletePublishedContent() {
    ...
    const confirmed = await showConfirm({
      title: "删除已发布书稿",
      content: `删除后读者端将无法再打开《${draft.title}》，且不可恢复。确定删除吗？`,
      confirmText: "删除"
    });
    if (!confirmed) return;
    ...
    this.actionRequestId = operationId;
    this.setData({
      busyAction: "deletePublishedContent",
      pageError: "",
      actionMessage: "",
      serverIssues: []
    });
    ...
    try {
      const result = await callAdmin("deletePublishedContent", {
        contentId: draft.targetId
      });
      if (this.pageDestroyed || operationId !== this.actionRequestId) return;
      if (!result.success) {
        this.setData({
          pageError: result.message || "删除失败，请稍后重试。",
          serverIssues: Array.isArray(result.issues)
            ? result.issues
                .map((issue) => normalizeText(issue, 180))
                .filter(Boolean)
            : []
      ...
      this.setData({
        actionMessage: "已删除已发布书稿，读者端将无法再打开这篇内容。"
      });
      ...
      if (!this.pageDestroyed && operationId === this.actionRequestId) {
        this.setData({
          pageError: getErrorMessage(error, "删除失败，请稍后重试。"),
          serverIssues: []
        });
      }
    } finally {
